...
def main():
    ...
    while numfw == 1 or numfw == 2:
        ...
        def ins():
            ...
            def gotolink(event):
                url = webbrowser.open('https://recordmp3online.com/')
                window.iconify()
                player.iconify()
                b["fg"] = "purple"
            def un(event):
            ...
            def nun(event):
                b["font"] = "bold 15"
            a = Label(window, text = "This is an Audio player.\n\nHow to use it: store all your .mp3 files in the same folder as the Python file. Then open the file and start!\nHint: Python only supports .mp3 files.\nIf you want to record and convert your files; Check this Link:", font = "bold 15")
        ...
        list1 = listdir()
        for i in list1:
            if (not(i.endswith(".mp3")))and(not("."in i)):
                list1.remove(i)
        if len(list1) == 0:
